...
| `privateApi.tls.secretName`               | `nil`                                                 | Name of the [Kubernetes TLS secret](https://kubernetes.io/docs/concepts/configuration/secret/#tls-secrets) which contains the certificate and its associated key. Required if `privateApi.tls` is `true`. |
